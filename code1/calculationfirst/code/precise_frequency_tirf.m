...
for ii=1:a_num
    ...
    mytemp=sum(sum(conj(norm_ft(:,:,ii,1)).*circshift(squeeze(norm_ft(:,:,ii,2)),[center_x,center_y])));%phase correlation
    test_ini_phase(1,ii)=angle(mytemp);%angle of phase shift in the article
