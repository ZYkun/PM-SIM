...
%          norm_ft(:,:,ii,jj)=ft_im(:,:,ii,jj);
        im(:,:,ii,jj)=fftshift(fft2(norm_ft(:,:,ii,jj)));%relative position
    end
    reference(:,:,ii)=fftshift(fft2(ifftshift(norm_ft(:,:,ii,1))));%zero position x=0,y=0   but not good to watch
end
% figure;
% imshow(abs(norm_ft(:,:,1,1)));
% title('im ');
% figure;
% imshow(abs(ifftshift(norm_ft(:,:,1,1))));
% title('reference');
...
% figure;
% imshow(abs(re_f(:,:,1,1))/max(max(abs(re_f(:,:,1,1)))));
% title('process freq district 0 order');
% figure;
% imshow(abs(re_f(:,:,1,2))/max(max(abs(re_f(:,:,1,2)))));
% title('process freq district 1 order ');
% figure;
% imshow(abs(re_f(:,:,1,3))/max(max(abs(re_f(:,:,1,3)))));
% title('process freq district -1 order');
% 
% figure;
% imshow(abs(re_f(:,:,2,1))/max(max(abs(re_f(:,:,1,1)))));
% title('process freq district 0 order');
% figure;
% imshow(abs(re_f(:,:,2,2))/max(max(abs(re_f(:,:,1,2)))));
% title('process freq district 1 order ');
% figure;
% imshow(abs(re_f(:,:,2,3))/max(max(abs(re_f(:,:,1,3)))));
% title('process freq district -1 order');
% 
% figure;
% imshow(abs(re_f(:,:,3,1))/max(max(abs(re_f(:,:,1,1)))));
% title('process freq district 0 order');
% figure;
% imshow(abs(re_f(:,:,3,2))/max(max(abs(re_f(:,:,1,2)))));
% title('process freq district 1 order ');
% figure;
% imshow(abs(re_f(:,:,3,3))/max(max(abs(re_f(:,:,1,3)))));
% title('process freq district -1 order');
